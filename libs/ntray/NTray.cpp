...
CTrayNotifyIcon::CTrayNotifyIcon() noexcept : m_bCreated(false),
                                              m_bHidden(false),
                                              ...
                                              m_nCurrentIconIndex(0)
{
  ...
  m_NotifyIconData.cbSize = sizeof(NOTIFYICONDATA_4);
}
...
#pragma warning(suppress: 26490)
    bSuccess = (Shell_NotifyIcon(NIM_DELETE, reinterpret_cast<PNOTIFYICONDATA>(&m_NotifyIconData)) != FALSE);
    m_bCreated = false;
  }
...
#pragma warning(suppress: 26490)
  const bool bSuccess = (Shell_NotifyIcon(NIM_ADD, reinterpret_cast<PNOTIFYICONDATA>(&m_NotifyIconData)) != FALSE);
  if (bSuccess)
  {
    m_bCreated = true;
...
#pragma warning(suppress: 26477)
  ...
  m_NotifyIconData.dwStateMask = NIS_HIDDEN; 
#pragma warning(suppress: 26490)
  const bool bSuccess = (Shell_NotifyIcon(NIM_MODIFY, reinterpret_cast<PNOTIFYICONDATA>(&m_NotifyIconData)) != FALSE);
  if (bSuccess)
    m_bHidden = true;
  return bSuccess;
}
...
#pragma warning(suppress: 26490)
  m_bCreated = (Shell_NotifyIcon(NIM_ADD, reinterpret_cast<PNOTIFYICONDATA>(&m_NotifyIconData)) != FALSE);
  if (m_bCreated)
  {
    if (!bShow)
      m_bHidden = true;
...
#pragma warning(suppress: 26490)
  return Shell_NotifyIcon(NIM_SETVERSION, reinterpret_cast<PNOTIFYICONDATA>(&m_NotifyIconData)) != FALSE;
}
...
#ifdef _AFX
#pragma warning(suppress: 26434)
bool CTrayNotifyIcon::Create(_In_ CWnd* pNotifyWnd, _In_ UINT uID, _In_ LPCTSTR pszTooltipText, _In_ const CBitmap* pBitmap, _In_ UINT nNotifyMessage, _In_ UINT uMenuID, _In_ bool bShow)
#else
#pragma warning(suppress: 26434)
bool CTrayNotifyIcon::Create(_In_ CWindow* pNotifyWnd, _In_ UINT uID, _In_ LPCTSTR pszTooltipText, _In_ const CBitmap* pBitmap, _In_ UINT nNotifyMessage, _In_ UINT uMenuID, _In_ bool bShow)
#endif //#ifdef _AFX
{
...
bool CTrayNotifyIcon::SetBalloonDetails(_In_ LPCTSTR pszBalloonText, _In_ LPCTSTR pszBalloonCaption, _In_ BalloonStyle style, _In_ UINT nTimeout, _In_ HICON hUserIcon, _In_ bool bNoSound, _In_ bool bLargeIcon, _In_ bool bRealtime, _In_ HICON hBalloonIcon) noexcept
{
  if (!m_bCreated)
    return false;
...
#pragma warning(suppress: 26490)
  return (Shell_NotifyIcon(NIM_MODIFY, reinterpret_cast<PNOTIFYICONDATA>(&m_NotifyIconData)) != FALSE);
}
...
bool CTrayNotifyIcon::SetTooltipText(_In_ LPCTSTR pszTooltipText) noexcept
{
  if (!m_bCreated)
    return false;
...
bool CTrayNotifyIcon::SetFocus() noexcept
{
  //Call the Shell_NotifyIcon function
#pragma warning(suppress: 26490)
  return (Shell_NotifyIcon(NIM_SETFOCUS, reinterpret_cast<PNOTIFYICONDATA>(&m_NotifyIconData)) != FALSE);
}
